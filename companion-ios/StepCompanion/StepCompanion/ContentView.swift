...
struct ContentView: View {
    ...
    @AppStorage("hostText") private var hostText = "192.168.1.x"
    @AppStorage("portText") private var portText = "8787"
    @AppStorage("deviceLabel") private var deviceLabel = ""
    @AppStorage("deviceUDID") private var deviceUDID = ""
    ...
    var body: some View {
        NavigationStack {
            Form {
                ...
                Section("Device") {
                    TextField("Label", text: $deviceLabel)
                        .autocorrectionDisabled()
                    TextField("UDID (optional)", text: $deviceUDID)
                        .autocorrectionDisabled()
                    ...
                    Button(client.connected ? "Disconnect" : "Connect") {
                        if client.connected {
                            ...
                            client.connect(
                                url: url,
                                writer: writer,
                                label: deviceLabel.isEmpty ? "iPhone" : deviceLabel,
                                udid: deviceUDID.isEmpty ? nil : deviceUDID
                            )
                        }
                    }
